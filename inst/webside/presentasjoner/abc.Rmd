...
dkeykg <- ggplot(data = ddatainp, aes(x = x, y = value, color = factor(variable))) + 
  ...
  geom_segment(aes(x = 0, y = yeae, xend = yeae , yend = yeae), lty = 2) + 
  geom_text(x=550, y=600, label = 'Y = AD', color = 'red') +
  geom_text(x=550, y=550, label = 'C+I+G', color = 'red') +
...
#### Kort sikt (0-3 år):
- Pga av antagelsen om at produktpriser og lønninger ikke endrer seg på kort sikt (og statiske prisforventninger), vil  samlet etterspørsel ($AE$) bestemmer BNP ($Y$)
...
Økte investeringer $\rightarrow$ økt produksjon $\rightarrow$ initierer en multiplikatoreffekt: $\rightarrow$ økt disponibel inntekt $\rightarrow$ økt konsum $\rightarrow$ økt produksjon etc.
