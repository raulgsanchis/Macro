...
names <- unique(moltmacroopen$variable)
nordata <- dplyr::filter(moltmacroopen, land =='nor' & variable %in% c('unem', 'gdp', 'pricei'))
gerdata <- dplyr::filter(moltmacroopen, land =='ger')
gredata <- dplyr::filter(moltmacroopen, land =='gre')

abc <- reshape2::dcast(nordata, date ~ variable + land)
rawmoltmacroopen <- rbind(nordata, gerdata, gredata)

sti <- paste0(devtools::as.package(".")$path,'/inst/webside/data/','tidserieopenokonomi.xlsx')
openxlsx::write.xlsx(rawmoltmacroopen, file=sti)
