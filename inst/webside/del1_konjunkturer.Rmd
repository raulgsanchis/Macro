...
b <- unique(dfkeynes$dfmodell$variable)

kompanalyse <- gpdiamdoell3(ndata = dfkeynes$dfmodell,
                            variables = c('grad45','cdvpidvgdv'),
...
keykryss <- gpdiamdoell(data = dfkeynes$dfmodell, 
                        variables = c("grad45v", "idv", "gdvpidv", "cdvpidvgdv"),
                        labt = lableg,
                        color = rep(c('black'),4),
                        labplassmon = labldf,
...
ckeykryss <- gpdiamdoell(data = cdfkeynes$dfmodell, 
                         variables = c("grad45v","cdv"),
                         labt = lableg,
                         color = rep(c('black'), 2),
                         labplassmon = clabldf,
                         ...
                         odata = dfkeynes$dfmodell,
                         ovariables = c('cdvpidvgdv'))

onel <-gponeliner(data = dfkeynes$dfmodell, navnvar = c("cdvpidvgdv"))
...
## Labeling
lableg <- list(title= 'Pengemarkedet', x='M/P', 
                                       y='i')
...
figpeng <- gpdiamdoell2(data = dfislm$dfmodell, variables = c("ldv", "msv"),
                           labt = lableg,
                           color = rep(c('black'),2))
...
figislm <- gpdiamdoell2(data = dfislm$dfmodell, variables = c("isv", "lmv"),
                           labt = lableg,
                           color = rep(c('black'),2))
...
cfigislm <- gpdiamdoell2(data = cdfislm$dfmodell, variables = c("isv", "lmv"),
                           labt = lableg,
                           color = rep(c('black'),2))
```
...
```{r}
grid.arrange(figpeng, figislm , ncol= 2, top = 'Økning i pengemengden Mo' )
```
