...
usadataI <- dplyr::filter(moltmacrousa, variable %in% c('unem', 'cunem', 'ggdp', 'inflation', 'cinflation'))
...
philipusa <- qplot(data = fusadataI, x = unem, y = cinflation, geom = c('point', 'smooth'), method='lm')


okunusag <- ggplot(data = fusadataI, aes(x = cunem, y = ggdp))  + geom_point() +  geom_smooth(method=lm, se=FALSE) + theme_classic() + labs(title = 'Okuns lov', x = 'Endring i arbeidsledigheten', y = c('Vekst i BNP (realpriser)'))

philipusag <- ggplot(data = fusadataI, aes(x = unem, y = inflation))  + geom_point() +  geom_smooth(method=lm, se=FALSE) + theme_classic() + labs(title = 'Phillips-kurven' ,x = 'Ledighetsrate', y = 'Inflation')
...
# Datatabell (format)
dfkeykryss <- data.frame(x = Yv, grad45 = grad45v, cd = cdv) %>% 
  reshape2::melt(id.vars = c("x")) 
  #%>%dplyr::mutate(kat=substring(variable,6,6))

# Qplot
 #unique(dfkeykryss$variable)
datainp <- dplyr::filter(dfkeykryss, variable %in% c('grad45', 'cd'))
keykg <- ggplot(data = datainp, aes(x = x, y = value, color = factor(variable))) + geom_line() +
...
1. Intern balanse - lukke produksjonsgapet, $(Y-\overline{Y})=0$

### Sentrale nasjonalregnskapstall
- Bruttonasjonalprodukt (GDP)
Den samlede verdien av alle varer og tjenester som blir produsert i landet i løpet av et år
- Nettonasjonalprodukt (GNP)
Den samlede verdien av alle varer og tjenester som landets innbyggere produserer i landet i løpet av et år
- Arbeidsledighet: $u = 1-\frac{L}{N}$
- Inflasjon: $\pi = \frac{P_{1} - P_{0}}{P_{0}}$

---

### Sentrale empiriske sammenhenger
#### Okuns lov - negativ korrelasjon mellom endring i ledighet og vekst i BNP (realpriser) 
- USA fra 1950-2010
```{r echo=FALSE}
okunusag 
```
...
#### Phillips-kurven (sammenheng mellom arbeidsledighet og inflasjon eller endring i inflasjon)
- USA fra 1950-2010
...
#### Kort sikt - Keynes, IS-LM og Mundell-Fleming modellen i del I og IS-LM for to husholdninger i del III (0-3 år):
- Samlet etterspørsel ($AE$) bestemmer BNP ($Y$)

#### Mellomlang sikt - AD-AS modelen i del I (5-10 år):
- Tilgangen på innsatsfaktorene arbeidskraft ($L$), teknkologi ($A$) og kapital ($K$) bestemmer BNP ($Y$)

#### Lang sikt - Solow modellen i del II (10-50 år):
...
#### Løsning av modellen:
Setter adferdslikningene inn i markedslikevekten
...
-  Grafisk framvisning ('Keynesian cross')
...
I praksis kan det ofte være vanskelig å benytte finanspolitske som konjunkturstabiliserende virkemiddel, siden det politiske systemet vanligvis innehar tregheter tilknyttet endringer i finanspolitikken
