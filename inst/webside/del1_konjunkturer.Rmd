...
islmequ <- rjson::fromJSON(file=paste0(devtools::as.package(".")$path,'/inst/webside/jupyter/islmequ.json'))

# Parameterverdier (Keyneskrysset)
exoparval <- list(c_1 = 0.6, oC = 25, oG= 50, a = 30, i = 0.03, oI = 50, T = 10)
...
#
dexoparval <- list(c_1 = 0.6, oC = 25, oG= 50, a = 30, i = 0.03, oI = 110, T = 10)
...
## Henter datasettet
datakeynes <- dplyr::filter(dfkeykryss, variable %in% c('grad45', 'id', 'idpgd', 'idpgdcd'))
## Komponent1
...
#### Løsning av modellen:
Setter alle adferdsligningene inn i likevekstbetingelsen for godemarkedet
...
#### Løsning av modellen:
#### Godemarkedet (samme løsning som under enkel Keynes-modell)
$$Y = \frac{1}{1-c}  \left [ \overline{C} -c\cdot \overline{T} - b\cdot \overline{i} + \overline{G}  \right ]$$
...
dfkeynes <- dfgpmakro(Yv, exoparval)
cdfkeynes <- dfgpmakro(Yv, cexoparval)

labldf <- data.frame(labeling=c('45 grader', 'I', 'G + I', 'Y = AE', 'C+ I + G'), 
                     x=c(50, 560, 560, 560, 560), 
                     y = c(20, 60, 110, 490, 600),
                     col = c('black', rep(c('red'),1,4)))

clabldf <- data.frame(labeling=c('45 grader', 'Y = AE', 'C+ I + G'), 
                     x=c(50, 560, 560), 
                     y = c(20, 490, 600),
