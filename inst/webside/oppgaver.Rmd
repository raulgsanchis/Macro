...
library(dplyr)
moltmacroopen <- rbind(MakroOEKB1115::moltmacronor,MakroOEKB1115::moltmacrohellas, MakroOEKB1115::moltmacroger)

unique(moltmacroopen$variable)

pmoltmacroopen <- moltmacroopen %>% dplyr::filter(variable %in% c('gdp','ggdp', 'unem', 'pricei', 'inflation', 'cinflation'))

datainp <- dplyr::filter(moltmacroopen, variable %in% c('unem'), land =='nor')
datainp <- dplyr::filter(moltmacroopen, variable %in% c('gdp'), land =='nor')
datainp <- dplyr::filter(moltmacroopen, variable %in% c('pricei'), land =='nor')

a <- ggplot(data = datainp, aes(x = date, y =  value)) + geom_line(aes(color = variable))
b <- a + theme_bw()
```
