...
\title[]{Likviditetsfellen}
\author[]{J\o rn Inge Halvorsen }
\date[]{14. mai, 2014}
\institute[J\o rn Inge Halvorsen]{Presentasjon for H\o gskolen i Oslo}
\maketitle
\section{Bakgrunn}
\begin{frame}
\frametitle{Hva er likviditetsfellen?}
\begin{itemize}
\item I mange industrialiserte land har styringsrentene til sentralbankene i det siste n\ae rmet seg null
\includegraphics[scale=2]{styringsrente.png}\end{itemize}
\end{frame}
\begin{frame}
\frametitle{}
\begin{itemize}
\item En \o konomi befinner seg i en likviditetsfelle n\aa r det kortsiktige renteniv\aa et er (tiln\ae rmet) lik null
\item I denne situasjonen vil
\begin{itemize}
 \item Penger og obligasjoner v\ae re tiln\ae rmet  perfekte substitutter
 \item En \o kning i pengemengden vil ikke lengre ha en ekspansiv effekt p\aa \ \o konomien 
 \end{itemize}
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{\AA rsaker til likviditetsfellen}
\begin{enumerate}
\item Demografiske endringer\\
Krugman (1998), BPEA 
\item Krav om privat gjeldsreduksjon (Minsky-bevegelse)\\
Krugman og Eggertsson (2012), QJE
\end{enumerate}
\end{frame}
\begin{frame}
\frametitle{(1) Demografiske endringer}
\begin{itemize}
\item Generalbudsjettlikningen og likevektsbetingelsen for en lukket \o konomi \begin{equation*}
Y=C+\overline{I}+G
\end{equation*}
\item Konsumfunksjonen
\begin{equation*}
C=\bar{C}+c\cdot (1-t)\cdot Y-b\cdot i 
\end{equation*}
\item Demografiske endringer (pga. aldring) $\Delta \bar{C}<0$ . Dette gir f\o lgende effekt p\aa \ BNP
\begin{equation*}
...
\begin{frame}
\begin{itemize}
\item Fors\o ke \aa \ motvirke den negative effekten gjennom \aa \ sette renta ned til  null, $\Delta i ^{0}<0$
\begin{equation*}
\Delta Y=\frac{1}{1-c\cdot (1-t)}(\Delta\bar{C}-b\cdot \Delta i ^{0})<0
\end{equation*}
\item Vi har havnet i en likviditetsfelle dersom 
\begin{equation*}
(\Delta\bar{C}-b\cdot \Delta i^{0})<0
\end{equation*}
\item Totaleffekten p\aa \  BNP m\aa \ derfor bli  negativ
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{(2) Krav om privat gjeldsreduksjon (Minsky-bevegelse)}
\begin{itemize}
\item Generalbudsjettlikningen og likevektsbetingelsen for en lukket \o konomi
\footnote{Her er dette uttrykket delt p\aa \ antall innbyggere i \o konomien}
\begin{equation*}
Y=C+\overline{I}+G
\end{equation*}
\item To konsumenter 
...
\end{equation*}
\begin{equation*}
C^{b}= (1-t)\cdot Y-D^{\text{h\o y}}\cdot i_{-1}+\Delta D^{b} 
\end{equation*}
\begin{equation*}
C^{s}=\bar{C}^{s}+c\cdot (1-t)\cdot Y-b\cdot i 
\end{equation*}
\item Et krav om privat gjeldsreduksjon (Minsky-bevegelse), $\Delta D^{b} =(D^{lav}-D^{h\o y}<0)$. Dette gir f\o lgende effekt p\aa \ BNP  
\end{itemize}
...
\frametitle{}
\begin{itemize}
\item Fors\o ke \aa \ motvirke den negative effekten gjennom \aa \ sette renta til  null, $\Delta i ^{0}<0$

\begin{equation*}
\Delta Y=\frac{1}{1-\overline{c}(1-t)}\left[ \theta\Delta D^{b}-(1-\theta)b \Delta i^{0} \right]
\end{equation*}
\item Vi har havnet i en likviditetsfelle dersom 
\begin{equation*}
\left( \theta\Delta D^{b}-(1-\theta)b\cdot \Delta i^{0} \right)<0  
\end{equation*}
\item Totaleffekten p\aa \  BNP m\aa \ derfor bli  negativ
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{Sammenhengen mellom likviditetsfellen og en statlig gjeldskrise}
\begin{itemize}
\item Antar proporsjonal skattesats
\begin{equation*}
\Delta T=t \Delta Y   
\end{equation*}
\item (1) Demografiske endringer 
\begin{equation*}
\Delta T=t \frac{1}{1-c(1-t)}(\Delta\bar{C}-b\cdot \Delta i ^{0})<0   
\end{equation*}
\item (2) Krav om privat gjeldsreduksjon
\begin{equation*}
\Delta T=t \frac{1}{1-\overline{c}(1-t)}(\theta \Delta\bar{D}^{b}-(1-\theta)b\cdot \Delta i ^{0})<0   
\end{equation*}
\item
Den prim\ae re budsjettbalansen er gitt ved  
\begin{equation*}
\frac{G_{t}-T_{t}}{Y_{t}}
\end{equation*}
\end{itemize}
...
\begin{frame}
\frametitle{}
\includegraphics[scale=5]{economist.jpg}
\end{frame}

\begin{frame}
\frametitle{}
\includegraphics[width=\textwidth]{gjeldandelbnp.png}
\end{frame}
\begin{frame}
\frametitle{Likviditetsfellen og finanspolitikk: En gratis lunsj?\footnote{Det resultatet vil ikke holde i en \aa pen \o konomi med flytende valutkurs}}
\begin{itemize}
\item I modellen kan en ekspansiv finanspolitikk, $\Delta G>0$, benyttes for \aa \ redusere bortfallet i ettersp\o rselen  
\begin{equation*}
(\theta\Delta D^{b}-(1-\theta)b\cdot \Delta i ^{0})=-\Delta G    
\end{equation*}
Noe som betyr at 
\begin{equation*}
\Delta Y=\frac{1}{1-\overline{c}(1-t)}( \theta\Delta D^{b}-(1-\theta)b\cdot \Delta i ^{0}+\Delta G)=0    
\end{equation*}
\item Fordeler:
\begin{enumerate}
\item Ledige ressurser blir satt i aktivitet
\item Rentekostnadene for offentlig investeringer er lik null
\item Stabilserer prisene (utenfor modellen)
\end{enumerate}
\end{itemize}
...
\begin{itemize}
\item I\ de fleste industraliserte land har det kortsiktige renteniv\aa et beveget seg mot null 
\item Vi har sett at dette kan tilskrives to \aa rsaker: \\
(1) Demografiske endringer \\
(2) Krav om privat gjeldsreduksjon
\item Det kan fort oppst\aa en n\ae r sammenheng mellom likviditetsfellen og en statlig gjeldskrise  
\item I en likviditetsfelle for en lukket \o konomi, er en ekspansiv finanspolitikk n\ae rmest \aa\ betrakte som en gratis lunsj
\end{itemize}
\end{frame}
\end{document}
...
\end{document}
Krav om gjeldsreduksjon

\frametitle{\AA rsaker til likvidietsfellen}
\begin{enumerate}
\item Demografiske endringer
\item Krav om gjeldsreduksjon
\end{enumerate}
\end{frame}
\begin{frame}
\frametitle{\AA rsaker til }
Test
Test
Test
\end{frame}
...
\frametitle{}
\begin{figure}[h]
\caption{Penger utenfor landets grenser under Bretton Woods-systemet. Forårsaket her av en \o kning i basispenger.}
\begin{center}
